...
- The system parses the file, extracts transactions, and classifies each into income/expense/investment.
...
- Support for common statement formats from major Indian and international banks (initial launch scope to be defined with a supported-bank list).
...
- **AI Suggestions:** Integration with an LLM API (e.g., Claude via Anthropic API) for generating financial insights from aggregated, anonymized user data.
...
- **File Parsing:** Apache PDFBox / Apache POI for PDF/Excel statement parsing; OpenCSV for CSV
- **Security:** HTTPS everywhere, encryption at rest for sensitive fields (e.g., account numbers if stored), rate limiting, audit logging
